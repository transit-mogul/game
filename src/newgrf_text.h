...
/** Mapping of language data between a NewGRF and transit_mogul. */
struct LanguageMap {
	/** Mapping between NewGRF and transit_mogul IDs. */
	struct Mapping {
		byte newgrf_id;  ///< NewGRF's internal ID for a case/gender.
		byte transit_mogul_id; ///< transit_mogul's internal ID for a case/gender.
	};
	 ...
	 * the NewGRF's internal ID to transit_mogul's ID whereas for the choice lists we map
	 * the genders/cases/plural transit_mogul IDs to the NewGRF's internal IDs. In this
	...
	SmallVector<Mapping, 1> gender_map; ///< Mapping of NewGRF and transit_mogul IDs for genders.
	SmallVector<Mapping, 1> case_map;   ///< Mapping of NewGRF and transit_mogul IDs for cases.
	int plural_form;                    ///< The plural form used for this language.

	int GetMapping(int newgrf_id, bool gender) const;
	int GetReverseMapping(int transit_mogul_id, bool gender) const;
