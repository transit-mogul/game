...
struct LoggedChange {
	...
	union {
		...
		struct {
			char text[GAMELOG_REVISION_LENGTH]; ///< revision string, _transit_mogul_revision
			uint32 newgrf;   ///< _transit_mogul_newgrf_version
			uint16 slver;    ///< _sl_version
			byte modified;   ///< _transit_mogul_revision_modified
		} revision;
		struct {
